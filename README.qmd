...
Standard Git + GitHub, but to keep the repo size small we don't need to track the history of the raw data object. We therefore just delete it from git history periodically:

1. Remove data file from git history: `git filter-repo --path data/preprints.rda --invert-paths --force`
2. Clean up git storage: `git reflog expire --expire=now --all && git gc --prune=now --aggressive`
3. Re-add origin remote: `git remote add origin https://github.com/mvuorre/psyarxivr.git`
